...
async def generate_course(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("Python HTTP trigger function processed a request." + os.environ.get("ATHENA_URL"))
    ...
    if not query:
        return func.HttpResponse(
            json.dumps(
                {
                    "message": "This HTTP trigger function needss a query with preferences in the request body for a personalized response."
                }
            ),
    ...
    try:  
        ...
        await save_course_to_db(result, query)
    ...
    try:
        ...
        async for message in team.run_stream(task=queryString):
            if (
                ...
                and message.source == "course_assembler"
            ):
