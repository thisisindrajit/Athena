...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <>
      ...
      <div className="footer fixed lg:hidden left-0 bottom-0 p-1.25 px-4 bg-background flex items-start justify-between gap-2 w-full border-t text-sm text-muted-foreground dark:text-foreground pb-safe h-[calc(3.5rem+env(safe-area-inset-bottom))]">
        {routes.map((route) => (
