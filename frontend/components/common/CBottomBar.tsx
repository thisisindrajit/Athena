...
const CBottomBar = () => {
  ...
  return isUserRoute ? (
    ...
    //   ))}
    // </div>
    <div className="lg:hidden dock bg-background border-t border-foreground/25 text-sm text-muted-foreground dark:text-foreground flex justify-between px-4">
      {routes.map((route) => (
        <Link href={route.href} key={route.href} className="min-w-[32%]">
          <div
            className={`w-full p-3 rounded-md flex items-center justify-center gap-2 font-medium ${
              route.active
                ? "text-primary bg-primary/10"
